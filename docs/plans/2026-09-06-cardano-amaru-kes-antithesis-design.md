...
The Antithesis Compose file must eventually contain public anonymous digest
references for every custom image. A local tag belongs only in a local override.
Until the exact built image is public, pinned, and pulled anonymously, the
package is not ready for submission. No launch is authorized by this design.
