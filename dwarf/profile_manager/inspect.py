...
LEGACY_RUNTIME_ROOT = "/home/USER/cardano-devnet"
...
PY
  ...
  if [ -S "$socket_path" ]; then
    CARDANO_NODE_SOCKET_PATH="$socket_path" /home/USER/.local/bin/cardano-cli query tip --testnet-magic 2 2>/dev/null || echo "tip_query=failed"
    echo
  else
  ...
  host_node_processes=$(ps -eo command 2>/dev/null | grep -Ec '[/]home/USER/.local/bin/cardano-node run' || true)
  ...
  if [ -S "$socket" ]; then
    CARDANO_NODE_SOCKET_PATH="$socket" /home/USER/.local/bin/cardano-cli query tip --testnet-magic 42 2>/dev/null || echo "tip_query=failed"
    echo
