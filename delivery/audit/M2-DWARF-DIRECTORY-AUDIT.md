...
- [x] Rebuild and redeploy on `<remote-host>`.
...
- Live deployed dashboard was verified at `http://<box-lan-ip>:8879` after
...
- Top-level directory scan found these public-delivery risk areas:
  ...
  - `dwarf/devnet-build/`: one Dockerfile with local/<remote-host> assumptions; referenced by `dwarf/profile_manager/profiles.py`.
  - `dwarf/grammars/`: useful for fuzz/dictionary work, but contains ledger/future dictionaries and an old <remote-host> sync note.
...
- Historical `dwarf/evidence/` and `dwarf/runs/` contain `<remote-host>` and `/home/USER` strings as recorded execution output. Those should not automatically be treated like public docs/code; they may be valid preserved evidence if the surrounding docs make that clear.
...
- Verification after cleanup:
  ...
  - `<remote-host>` delivery contract passed.
  - `<remote-host>` rebuilt and redeployed `dwarf/framework:june-20260604-m2`
...
for path in ["/operate", "/operate/targets", "/operate/runs", "/operate/bundles", "/api/status"]:
    with urllib.request.urlopen("http://<box-lan-ip>:8879" + path, timeout=5) as response:
