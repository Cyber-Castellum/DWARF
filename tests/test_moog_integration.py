...
def test_deployment_config_round_trips_moog_block():
    value = {
        "enabled": True,
        "deploy_root": "/home/USER/moog-deploy",
        "moog_binary": "/home/USER/bin/moog",
        "secrets_root": "/home/USER/moog-secrets",
        "mpfs_host": "https://mpfs.plutimus.com",
        "token_id": "<moog-token-id>",
    ...
    assert config.moog["deploy_root"] == "/home/USER/moog-deploy"
...
def test_moog_health_command_checks_public_state_without_reading_secrets():
    ...
    assert "/home/USER/moog-deploy" in command
    assert "/home/USER/bin/moog" in command
    ...
    assert "cat /home/USER/moog-secrets" not in command
...
def test_parse_moog_health_result_summarizes_remote_json():
    remote_payload = {
        "checks": [
            ...
            {"id": "deploy_root", "state": "ok", "detail": "/home/USER/moog-deploy"},
    ...
    result = CommandResult(
        ...
        stderr="",
        rendered_command="ssh <remote-host> moog-health",
    )
...
def test_moog_bootstrap_command_prepares_dirs_without_reading_or_starting_secrets():
    ...
    assert "/home/USER/moog-deploy" in command
    assert "/home/USER/moog-secrets" in command
    assert "requester-wallet-info.json" in command
    assert "oracle-wallet-info.json" in command
    assert "cat /home/USER/moog-secrets" not in command
    assert "requester.json" not in command
...
def test_parse_moog_bootstrap_result_summarizes_remote_json():
    remote_payload = {
        ...
        "checks": [
            {"id": "deploy_root", "state": "ok", "detail": "/home/USER/moog-deploy"},
            {"id": "binary", "state": "warn", "detail": "/home/USER/bin/moog"},
        ],
    ...
    result = CommandResult(
        ...
        stdout=json.dumps(remote_payload),
        stderr="",
        rendered_command="ssh <remote-host> moog-bootstrap",
    )
            ...
            stdout=json.dumps(
                {
                    "state": "ok",
                    "applied": True,
                    "checks": [{"id": "deploy_root", "state": "ok", "detail": "/home/USER/moog-deploy"}],
                    "healthcheck_plan": [{"command": "cardano-profile moog healthcheck --json"}],
                }
            ),
            stderr="",
...
def test_moog_create_test_command_uses_requester_wallet_without_reading_secret():
    ...
    assert "-w /home/USER/moog-secrets/requester/requester.json" in command
    ...
    assert "cat /home/USER/moog-secrets/requester/requester.json" not in command
...
def test_moog_registration_submit_command_uses_wallet_without_reading_secret():
    ...
    config = {
        **DEFAULT_MOOG_CONFIG,
        "requester_wallet_file": "/home/USER/moog-secrets/requester/requester.json",
    }
    plan = build_moog_registration_plan(
        moog_config=config,
        health={
...
def test_operate_status_moog_tile_uses_summary_and_checks():
    ...
    assert tile["deploy_root"] == "/home/USER/moog-deploy"
